...
public class WPS_New_Hire_Main {
	GenericMethods2 gm = new GenericMethods2();
	common_methods cm = new common_methods();
	WPS_New_Hire_Test WSNT = new WPS_New_Hire_Test();
	...
	public  void Menu_Navigation() throws Exception 
	{
		try
		{
			WSNT.go_to_Main_Page(common_methods.driver,logger);
			Thread.sleep(10000);
		}
		catch(Exception e)
		{
	...
	@Test(priority=2,dataProvider="XNARF_WPS_TASK")
	public void Xnarf_WPS(String flag, String action_type,String Request_id,String First,String Personnel_Number, String comments) throws Exception 
	{
		try
		{	
			if(flag.equalsIgnoreCase("y") && action_type.equals("Single"))
			{
				try
				{
				 WSNT.WPS_Single(common_methods.driver, Request_id,Personnel_Number, comments, logger);
				}
				catch(Exception e)
				{
					gm.logFail(logger, "Exception occured"+e,common_methods.driver);
			...
			else if(flag.equalsIgnoreCase("y") && action_type.equals("Multiple"))
			{
				try
				{
					WSNT.WPS_Multiple(common_methods.driver, Request_id,First, Personnel_Number ,comments,  logger);
				}
				catch(Exception e)
				{
					gm.logFail(logger, "Exception occured"+e,common_methods.driver);
			...
			else if(flag.equalsIgnoreCase("y") && action_type.equals("MultipleComplete"))
				{
					try
					{
						WSNT.MultipleComplete(common_methods.driver, Request_id,First, Personnel_Number ,comments,  logger);
					}
					catch(Exception e)
					{
						gm.logFail(logger, "Exception occured"+e,common_methods.driver);
	...
	@DataProvider(name="XNARF_WPS_TASK")
	public Object[][] getData()
	{
		Object[][] c1 = gm.getExcelData(EnvironmentVariables.dataPoolPath, "Xnarf_WPS");
		return c1;		
	}
